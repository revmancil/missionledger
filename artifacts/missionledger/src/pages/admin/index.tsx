...
function OrgDetailDrawer({ org, onClose, onRefresh }: { org: OrgRow; onClose: () => void; onRefresh: () => void }) {
  ...
  async function handleResetPassword(e: React.FormEvent) {
    ...
    setResetMsg(null);
    try {
      const data = await apiFetch("/api/master-admin/reset-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: resetUserId, newPassword: newPw }),
      });
      setResetMsg({ ok: true, text: data.message ?? "Password reset successfully." });
      setNewPw("");
      setResetUserId(null);
    } catch (err: any) {
      setResetMsg({ ok: false, text: err.message ?? "Failed to reset password." });
    } finally {
      setResetting(false);
    }
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex">
      ...
      {/* Panel — explicit h-screen so footer never falls off */}
      <div className="w-[500px] h-screen bg-slate-900 border-l border-slate-800 flex flex-col shadow-2xl">
        {/* Header */}
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-6 space-y-6">
          ...
          {/* Global reset success/error banner */}
          {resetMsg && (
            <div className={cn(
              ...
              {resetMsg.ok ? <CheckCircle2 className="h-4 w-4 shrink-0 mt-0.5" /> : <AlertTriangle className="h-4 w-4 shrink-0 mt-0.5" />}
              {resetMsg.text}
            </div>
          )}
          ...
          <div>
            <h4 className="text-[11px] text-slate-500 uppercase tracking-wide mb-3">Team Members & Password Reset</h4>
            {!detail ? (
                  ...
                  <div key={u.id}>
                    ...
                    <div className="flex items-center justify-between px-4 py-2.5">
                      <div className="min-w-0">
                        <div className="text-sm text-slate-200 font-medium truncate">{u.name || u.email}</div>
                        <div className="text-xs text-slate-500 truncate">{u.email}</div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0 ml-2">
                        <span className="text-[10px] font-mono text-slate-500 bg-slate-800 px-2 py-0.5 rounded">{u.role}</span>
                        <span className={cn("w-1.5 h-1.5 rounded-full shrink-0", u.isActive ? "bg-emerald-500" : "bg-slate-600")} />
                        <button
                    ...
                    {resetUserId === u.id && (
                      <form
                        ...
                        className="px-4 pb-3 pt-1 bg-slate-800/40 border-t border-slate-700/50"
                      >
                        <p className="text-[11px] text-slate-500 mb-2">
                          Set a new password for <span className="text-slate-300 font-medium">{u.email}</span>
                        </p>
                        <div className="flex items-center gap-2">
                          <div className="relative flex-1">
                            <input
                              ...
                              minLength={8}
                              required
                              className="w-full h-8 px-3 pr-8 rounded-lg bg-slate-800 border border-slate-700 text-slate-200 text-xs focus:outline-none focus:ring-2 focus:ring-amber-700"
                            />
                            <button
                              type="button"
                          ...
                          <button
                            ...
                            className="h-8 px-3 rounded-lg bg-amber-700 hover:bg-amber-600 disabled:opacity-50 text-white text-xs font-semibold flex items-center gap-1 transition-colors shrink-0"
                          >
                            {resetting ? <RefreshCw className="h-3 w-3 animate-spin" /> : <KeyRound className="h-3 w-3" />}
        ...
        <div className="shrink-0 px-6 py-4 border-t border-slate-800 flex items-center gap-3 bg-slate-900">
          <button

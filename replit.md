...
- **Plaid Bank Linking**: Each bank account card on `/bank-accounts` has a "Link Bank via Plaid" button. On click, fetches a Plaid Link token (`POST /api/plaid/create-link-token`), opens Plaid Link modal, exchanges the public token (`POST /api/plaid/exchange-token`), stores `plaidAccessToken` + `plaidItemId` + `plaidInstitutionName` + `isPlaidLinked` on the bank account. Sync button (`POST /api/plaid/sync/:id`) fetches 90 days of transactions. Unlink via `DELETE /api/plaid/unlink/:id`. **Environment**: Set `PLAID_CLIENT_ID` and `PLAID_SECRET` from the Plaid Dashboard for the target environment. Optional `PLAID_ENV` = `sandbox` | `development` | `production` (defaults: if unset and `NODE_ENV=production`, Plaid **production** is used; otherwise **sandbox**). Replit shared env sets `PLAID_ENV=production`; use Plaid **production** secret there. For local sandbox testing, set `PLAID_ENV=sandbox` and sandbox credentials.
...
- **Platform Admin Command Center** (`/admin`): Separate view (guarded by `isPlatformAdmin` flag on users) showing all organizations across the platform with stats, user counts, subscription status. Supports: suspend/activate orgs (which blocks all logins for that org), impersonate (view any org's books as a support admin with a 2-hour session), and org detail/user drill-down. Legacy path `/master-admin` redirects to `/admin`.
